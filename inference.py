...
from trainer.metrics import (Rsquared, MeanPredictorLoss, MAE,
                            PearsonR, RMSD, RMSDfraction, CentroidDist,
...
def parse_arguments(arglist = None):
    ...
    parser.add_argument('--config', type=argparse.FileType(mode='r'),
                    default='configs_clean/inference.yml')
    parser.add_argument('--checkpoint', type=str, help='path to .pt file in a checkpoint directory')
    parser.add_argument('--output_directory', type=str, default=None,
                    help='path where to put the predicted results')
    parser.add_argument('--run_corrections', type=bool, default=False,
                   help='whether or not to run the fast point cloud ligand fitting')
    parser.add_argument('--run_dirs', type=list, default=[], help='path directory with saved runs')
    parser.add_argument('--fine_tune_dirs', type=list, default=[],
                    help='path directory with saved finetuning runs')
    parser.add_argument('--inference_path', type=str,
                    help='path to some pdb files for which you want to run inference')
    parser.add_argument('--experiment_name', type=str,
                    help='name that will be added to the runs folder output')
    parser.add_argument('--logdir', type=str, default='runs', help='tensorboard logdirectory')
    ...
    parser.add_argument('--minimum_epochs', type=int, default=0,
                    help='minimum numer of epochs to run')
    parser.add_argument('--dataset_params', type=dict, default={},
    ...
    parser.add_argument('--multithreaded_seeds', type=list, default=[],
                   help=('if this is non empty, multiple threads will be started, '
                   'training the same model but with the different seeds'))
    ...
    parser.add_argument('--loss_params', type=dict, default={},
                    help='parameters with keywords of the chosen loss function')
    parser.add_argument('--optimizer', type=str, default='Adam',
                    help='Class name of torch.optim like [Adam, SGD, AdamW]')
    parser.add_argument('--optimizer_params', type=dict,
                    help='parameters with keywords of the chosen optimizer like lr')
    parser.add_argument('--clip_grad', type=float, default=None,
                    help='clip gradients if magnitude is greater')
    parser.add_argument('--lr_scheduler', type=str,
                   help=('Class name of torch.optim.lr_scheduler like '
                   '[CosineAnnealingLR, ExponentialLR, LambdaLR]'))
    parser.add_argument('--lr_scheduler_params', type=dict,
                    help='parameters with keywords of the chosen lr_scheduler')
    parser.add_argument('--scheduler_step_per_batch', default=True, type=bool,
                   help='step every batch if true step every epoch otherwise')
    parser.add_argument('--log_iterations', type=int, default=-1,
                   help=('log every log_iterations iterations '
                   '(-1 for only logging after each epoch)'))
    parser.add_argument('--expensive_log_iterations', type=int, default=100,
                   help='frequency with which to do expensive logging operations')
    parser.add_argument('--eval_per_epochs', type=int, default=0,
                   help=('frequency with which to do run the function run_eval_per_epoch '
                   'that can do some expensive calculations on the val set or sth like that. '
                   'If this is zero, then the function will never be called'))
    parser.add_argument('--metrics', default=[], help=('tensorboard metrics '
                    '[mae, mae_denormalized, qm9_properties ...]'))
    parser.add_argument('--main_metric', default='loss', help='for early stopping etc.')
    parser.add_argument('--main_metric_goal', type=str, default='min',
                    help='controls early stopping. [max, min]')
    parser.add_argument('--val_per_batch', type=bool, default=True,
                   help=('run evaluation every batch and then average over the eval results. '
                   'When running the molhiv benchmark for example, this needs to be False '
                   'because we need to evaluate on all val data at once '
                   'since the metric is rocauc'))
    parser.add_argument('--tensorboard_functions', default=[],
                    help='choices of the TENSORBOARD_FUNCTIONS in utils')
    parser.add_argument('--num_epochs_local_only', type=int, default=1,
                   help=('when training with OptimalTransportTrainer, '
                   'this specifies for how many epochs only the local predictions '
                   'will get a loss'))
    parser.add_argument('--collate_function', default='graph_collate',
                    help='the collate function to use for DataLoader')
    parser.add_argument('--collate_params', type=dict, default={},
                   help='parameters with keywords of the chosen collate function')
    parser.add_argument('--device', type=str, default='cuda',
                    help='What device to train on: cuda or cpu')
    parser.add_argument('--models_to_save', type=list, default=[],
    ...
    parser.add_argument('--model_parameters', type=dict, help='dictionary of model parameters')
    parser.add_argument('--trainer', type=str, default='binding', help='')
    parser.add_argument('--train_sampler', type=str, default=None,
                    help='any of pytorchs samplers or a custom sampler')

    parser.add_argument('--eval_on_test', type=bool, default=True,
                    help='runs evaluation on test set if true')
    parser.add_argument('--check_se3_invariance', type=bool, default=False,
                    help='check it instead of generating files')
    parser.add_argument('--num_confs', type=int, default=1,
                    help='num_confs if using rdkit conformers')
    parser.add_argument('--use_rdkit_coords', action="store_true",
                   help='override the rkdit usage behavior of the used model')
    parser.add_argument('--no_use_rdkit_coords', action="store_false", dest = "use_rdkit_coords",
                   help='override the rkdit usage behavior of the used model')
...
def inference(args, tune_args=None):
    sys.stdout = Logger(
        logpath=os.path.join(os.path.dirname(args.checkpoint), 'inference.log'),
        syspart=sys.stdout)
    sys.stderr = Logger(
        logpath=os.path.join(os.path.dirname(args.checkpoint), 'inference.log'),
        syspart=sys.stderr)
    ...
    print('trainable params in model: ',
            sum(parser.numel() for parser in model.parameters() if parser.requires_grad))
    batch_size = args.batch_size if args.dataset_params['use_rec_atoms'] is False else 2
    ...
    for conformer_id in range(args.num_confs):
        ...
        for i, batch in tqdm(enumerate(loader)):
            with torch.no_grad():
                (lig_graphs,rec_graphs, ligs_coords, _, _,
                pockets_coords_lig, geometry_graph, names, idx) = tuple(batch)
                # if names[0] not in ['2fxs', '2iwx', '2vw5', '2wer', '2yge', ]: continue
                (ligs_coords_pred, ligs_keypts, recs_keypts,
                rotations, translations, _) = model(lig_graphs, rec_graphs,
                                                        complex_names=names,epoch=0,
                                                        geometry_graph=(geometry_graph.to(device)
                                                        if geometry_graph is not None else None))
                for (lig_coords_pred, lig_coords, lig_keypts,
                    rec_keypts, rotation, translation, rec_pocket_coords) in zip(
                            ligs_coords_pred, ligs_coords, ligs_keypts,
                            recs_keypts, rotations, translations, pockets_coords_lig):
                    all_ligs_coords_pred.append(lig_coords_pred.detach().cpu())
                    all_ligs_coords.append(lig_coords.detach().cpu())
                    all_ligs_keypts.append(((rotation @ (lig_keypts).T).T +
                                            translation).detach().cpu())
        ...
        path = os.path.join(os.path.dirname(args.checkpoint),
                            f'predictions_Tune{tune_args is not None}_RDKit{use_rdkit_coords}_confID{conformer_id}.pt')
        print(f'Saving predictions to {path}')
        results = {'predictions': all_ligs_coords_pred,
                    'targets': all_ligs_coords,
                    'lig_keypts': all_ligs_keypts,
                    'rec_keypts': all_recs_keypts,
                    'pocket_coords': all_pocket_coords,
                    'names': all_names}
        torch.save(results, path)
        rmsds = []
        centroid_distsH = []
        for i, (prediction, target, lig_keypts,
                rec_keypts, pocket_coords, name) in tqdm(enumerate(
                    zip(results['predictions'], results['targets'], 
                        results['lig_keypts'], results['rec_keypts'],
        ...
        print(f'''centroid_distances below 2: 
                {(100 * (centroid_distsH < 2).sum() / len(centroid_distsH)).__round__(2)}%''')
        print(f'''centroid_distances below 5: 
                {(100 * (centroid_distsH < 5).sum() / len(centroid_distsH)).__round__(2)}%''')
        ...
        if args.run_corrections:
            ...
            for i, (prediction, target, lig_keypts, rec_keypts, name) in tqdm(enumerate(
                    zip(results['predictions'], results['targets'], results['lig_keypts'],
                    results['rec_keypts'], results['names']))):
                lig = read_molecule(os.path.join('data/PDBBind/', name,
                                    f'{name}_ligand.sdf'), sanitize=True)
                if lig is None:  # read mol2 file if sdf file cannot be sanitized
                    lig = read_molecule(os.path.join('data/PDBBind/', name,
                                        f'{name}_ligand.mol2'), sanitize=True)
                ...
                for i in range(lig_rdkit.GetNumAtoms()):
                    x_coord, y_coord, z_coord = rdkit_coords[i]
                ...
                centroid_distance = np.linalg.norm(coords_native.mean(axis=0) -
                                    coords_pred.mean(axis=0))
                r_mat, t_mat = rigid_transform_Kabsch_3D(coords_pred.T, coords_native.T)
                moved_coords = (r_mat @ (coords_pred).T).T + t_mat.squeeze()
                ...
                for idx, rot in enumerate(rotable_bonds):
                    new_dihedrals[idx] = get_dihedral_vonMises(lig_rdkit,
                                        lig_rdkit.GetConformer(), rot, z_pt_cloud)
                ...
                rmsdval_optimized = np.sqrt(np.sum((coords_pred_optimized - coords_native) ** 2,
                                    axis=1).mean())
                centroid_distance_optimized = np.linalg.norm(
                    coords_native.mean(axis=0) - coords_pred_optimized.mean(axis=0))
                r_mat, t_mat = rigid_transform_Kabsch_3D(coords_pred_optimized.T, coords_native.T)
                moved_coords_optimized = (r_mat @ (coords_pred_optimized).T).T + t_mat.squeeze()
                kabsch_rmsd_optimized = np.sqrt(np.sum((moved_coords_optimized -
                                        coords_native) ** 2,axis=1).mean())
            ...
            print('mean kabsch RMSD: ', kabsch_rmsds.mean().__round__(2), ' pm ', kabsch_rmsds.std().__round__(2))
            ...
            print('mean kabsch RMSD: ', kabsch_rmsds_optimized.mean().__round__(2), ' pm ',
...
def inference_from_files(args):
    ...
    use_rdkit_coords = args.use_rdkit_coords if args.use_rdkit_coords is not None else args.dataset_params['use_rdkit_coords']
    names = os.listdir(args.inference_path) if args.inference_path is not None else tqdm(read_strings_from_txt('data/timesplit_test'))
    for idx, name in enumerate(names):
        ...
        for lig_name in lig_names:
            if not os.path.exists(os.path.join(args.inference_path, name, lig_name)):
                raise ValueError(f'''Path does not exist:
                        {os.path.join(args.inference_path, name, lig_name)}''')
            ...
            if lig is not None:  # read mol2 file if sdf file cannot be sanitized
                used_lig = os.path.join(args.inference_path, name, lig_name)
                break
        if lig_names == []:
            raise ValueError(f'No ligand files found. The ligand file has to contain \'ligand\'.')
        if lig is None:
            raise ValueError(f'None of the ligand files could be read: {lig_names}')
        ...
        lig_graph = get_lig_graph_revised(lig, name,
                                            max_neighbors=dataset_params['lig_max_neighbors'],
                                            use_rdkit_coords=use_rdkit_coords,
                                            radius=dataset_params['lig_graph_radius'])
        if 'geometry_regularization' in dataset_params and dataset_params['geometry_regularization']:
            geometry_graph = get_geometry_graph(lig)
        elif 'geometry_regularization_ring' in dataset_params and dataset_params['geometry_regularization_ring']:
        ...
        with torch.no_grad():
            ...
            (ligs_coords_pred_untuned, ligs_keypts, recs_keypts,
            rotations, translations, _) = model(
                lig_graph.to(device), rec_graph.to(device),
                geometry_graph, complex_names=[name], epoch=0)

            for (lig_coords_pred_untuned, lig_coords, lig_keypts,
                rec_keypts, rotation, translation) in zip(
                    ligs_coords_pred_untuned, [start_lig_coords],
                    ligs_keypts, recs_keypts, rotations,
                    translations, ):
                all_intersection_losses_untuned.append(
                    compute_revised_intersection_loss(
                        lig_coords_pred_untuned.detach().cpu(),
                        rec_graph.ndata['x'],
                        alpha=0.2, beta=8,
                        aggression=0))
            ...
            if args.run_corrections:
                ...
                for i in range(lig_input.GetNumAtoms()):
                    coord_x, coord_y, coord_z = input_coords.numpy()[i]
                    conf.SetAtomPosition(i, Point3D(float(coord_x), float(coord_y), float(coord_z)))

                lig_equibind = deepcopy(lig)
                conf = lig_equibind.GetConformer()
                for i in range(lig_equibind.GetNumAtoms()):
                    coord_x, coord_y, coord_z = prediction.numpy()[i]
                    conf.SetAtomPosition(i, Point3D(float(coord_x), float(coord_y), float(coord_z)))
                ...
                if args.output_directory:
                    ...
                    for i in range(optimized_mol.GetNumAtoms()):
                        coord_x, coord_y, coord_z = coords_pred_optimized[i]
                        conf.SetAtomPosition(i, Point3D(float(coord_x),
                                                float(coord_y),
                                                float(coord_z)))
                    block_optimized = Chem.MolToMolBlock(optimized_mol)
                    print(f'''Writing prediction to {args.output_directory}/
                                {name}/
    ...
    print(f'Saving predictions to {path}')
    results = {'corrected_predictions': all_ligs_coords_corrected,
                'initial_predictions': all_ligs_coords_pred_untuned,
                'targets': all_ligs_coords,
                'lig_keypts': all_ligs_keypts,
                'rec_keypts': all_recs_keypts,
                'names': all_names,
                'intersection_losses_untuned': all_intersection_losses_untuned,
