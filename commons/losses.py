...
# Intersection loss: IL = \avg_i max(0, surface_ct - G_r(a_i)) +
#                          \avg_j max(0, surface_ct - G_l(b_j))

def G_fn(protein_coords, x, sigma):
    # protein_coords: (n,3) ,  x: (m,3), output: (m,)
    e = torch.exp(- torch.sum((protein_coords.view(1, -1, 3) - x.view(-1, 1, 3)) ** 2, dim=2) \
                    / float(sigma))  # (m, n)
    return - sigma * torch.log(1e-3 + e.sum(dim=1))

def compute_body_intersection_loss(model_ligand_coors_deform,
                                    bound_receptor_repres_nodes_loc_array,
                                    sigma, surface_ct):
    loss = torch.mean(
        torch.clamp(surface_ct - G_fn(bound_receptor_repres_nodes_loc_array,
                                        model_ligand_coors_deform, sigma),min=0)) + \
           torch.mean(
               torch.clamp(surface_ct - G_fn(model_ligand_coors_deform,
                                            bound_receptor_repres_nodes_loc_array,
                                            sigma),min=0))
    return loss
...
def compute_revised_intersection_loss(lig_coords, rec_coords, alpha = 0.2, beta=8, aggression=0):
    ...
    distance_losses = aggression_term * \
                        torch.exp(-alpha*distances * torch.clamp(distances*4-beta, min=1))
    return distance_losses.sum()

class BindingLoss(_Loss):
    def __init__(self, ot_loss_weight=1, intersection_loss_weight=0, intersection_sigma=0,
                geom_reg_loss_weight=1, loss_rescale=True,
                intersection_surface_ct=0, key_point_alignmen_loss_weight=0,
                revised_intersection_loss_weight=0, centroid_loss_weight=0,
                kabsch_rmsd_weight=0,translated_lig_kpt_ot_loss=False,
                revised_intersection_alpha=0.1, revised_intersection_beta=8,
                aggression=0) -> None:
    ...
    def forward(self, ligs_coords, recs_coords, ligs_coords_pred,
                ligs_pocket_coords, recs_pocket_coords, ligs_keypts,
        ...
        for i in range(len(ligs_coords_pred)):
            ...
            if self.ot_loss_weight > 0:
                ...
                if self.translated_lig_kpt_ot_loss:
                    cost_mat_ligand = compute_sq_dist_mat(receptor_pocket_coors,
                                        (rotations[i] @ ligs_keypts[i].t()).t() + translations[i] )
                else:
            ...
            if self.key_point_alignmen_loss_weight > 0:
                keypts_loss += self.mse_loss((rotations[i] @ ligs_keypts[i].t()).t() +\
                                             translations[i],
                                             recs_keypts[i])

            if self.intersection_loss_weight > 0:
                intersection_loss = intersection_loss + \
                                    compute_body_intersection_loss(ligs_coords_pred[i],
                                                                    recs_coords[i],
            ...
            if self.revised_intersection_loss_weight > 0:
                intersection_loss_revised = intersection_loss_revised + \
                                    compute_revised_intersection_loss(ligs_coords_pred[i],
                                    recs_coords[i], alpha=self.revised_intersection_alpha,
            ...
            if self.kabsch_rmsd_weight > 0:
                ...
                A = (lig_coords_pred - lig_coords_pred_mean).transpose(0, 1) @ \
                    (lig_coords - lig_coords_mean)

                U, S, Vt = torch.linalg.svd(A)

                corr_mat = torch.diag(torch.tensor([1, 1, torch.sign(torch.det(A))],
                            device=lig_coords_pred.device))
                rotation = (U @ corr_mat) @ Vt
                translation = lig_coords_pred_mean - \
                                torch.t(rotation @ lig_coords_mean.t())  # (1,3)
                kabsch_rmsd_loss += self.mse_loss((rotation @ lig_coords.t()).t() + \
                                    translation, lig_coords_pred)
        ...
        return loss, {'ligs_coords_loss': ligs_coords_loss, 'recs_coords_loss': recs_coords_loss,
                        'ot_loss': ot_loss, 'intersection_loss': intersection_loss,
                        'keypts_loss': keypts_loss, 'centroid_loss:': centroid_loss,
                        'kabsch_rmsd_loss': kabsch_rmsd_loss,
                        'intersection_loss_revised': intersection_loss_revised,
