...
from commons.process_mols import get_rdkit_coords, get_receptor, get_pocket_coords, \
    read_molecule, get_rec_graph, get_lig_graph_revised, \
    get_receptor_atom_subgraph, get_lig_structure_graph, \
...
class PDBBind(Dataset):
    ...
    def __init__(self, device='cuda:0',
        ...
        self.conformer_id = 0
        if self.lig_predictions_name is None:
        ...
        if (not os.path.exists(os.path.join(self.processed_dir, 'geometry_regularization.pt')) and self.geometry_regularization) or \
            (not os.path.exists(os.path.join(self.processed_dir, 'geometry_regularization_ring.pt')) and self.geometry_regularization_ring) or \
            not os.path.exists(os.path.join(self.processed_dir, 'rec_graphs.pt')) or \
            not os.path.exists(os.path.join(self.processed_dir, 'pocket_and_rec_coords.pt')) or \
            not os.path.exists(os.path.join(self.processed_dir, self.lig_graph_path)) or \
            (not os.path.exists(os.path.join(self.processed_dir, self.rec_subgraph_path)) and self.rec_subgraph) or \
            (not os.path.exists(os.path.join(self.processed_dir, 'lig_structure_graphs.pt')) and self.lig_structure_graph):
            self.process()
        ...
        if self.multiple_rdkit_conformers:
            self.lig_graphs = [self.lig_graphs[i:i + self.num_confs] \
                                for i in range(0,len(self.lig_graphs), self.num_confs)]
        self.rec_graphs, _ = load_graphs(os.path.join(self.processed_dir, 'rec_graphs.pt'))
        if self.rec_subgraph:
            self.rec_atom_subgraphs, _ = load_graphs(os.path.join(self.processed_dir,
                                                    self.rec_subgraph_path))
        if self.lig_structure_graph:
            self.lig_structure_graphs, _ =  load_graphs(os.path.join(self.processed_dir,
                                                        'lig_structure_graphs.pt'))
            masks_angles = torch.load(os.path.join(self.processed_dir,
                                        'torsion_masks_and_angles.pt'))
        ...
        if self.geometry_regularization:
            ...
            self.geometry_graphs, _ =  load_graphs(os.path.join(self.processed_dir,
                                        'geometry_regularization.pt'))
        if self.geometry_regularization_ring:
            print(os.path.join(self.processed_dir,'geometry_regularization_ring.pt'))
            self.geometry_graphs, _ =  load_graphs(os.path.join(self.processed_dir,
                                                    'geometry_regularization_ring.pt'))
        ...
        if self.rec_subgraph:
            ...
            if self.random_rec_atom_subgraph:
                ...
                min_distances, _ = torch.cdist(rec_graph.ndata['x'],
                                    translated_lig_coords).min(dim=1)
                rec_graph = dgl.node_subgraph(rec_graph, min_distances < \
                                        self.random_rec_atom_subgraph_radius)
                assert rec_graph.num_nodes() > 0

        geometry_graph = None
        if self.geometry_regularization or self.geometry_regularization_ring:
            geometry_graph = self.geometry_graphs[idx]

        if self.lig_structure_graph:
            return (lig_graph.to(self.device), rec_graph.to(self.device), self.masks[idx],
                    self.angles[idx], lig_coords, rec_graph.ndata['x'], new_pocket_coords,
                    pocket_coords,geometry_graph, self.complex_names[idx], idx)
        return (lig_graph.to(self.device), rec_graph.to(self.device), lig_coords,
                rec_graph.ndata['x'], new_pocket_coords, pocket_coords, geometry_graph,
                self.complex_names[idx], idx)

    def process(self):
        log(f'''Processing complexes from [{self.complex_names_path}] and
                saving it to [{self.processed_dir}]''')
        ...
        for name in tqdm(complex_names, desc='loading ligands'):
            if self.bsp_ligands:
                lig = read_molecule(os.path.join(self.bsp_dir, name, 'Lig_native.pdb'),
                                    sanitize=True, remove_hs=self.remove_h)
                if lig is None:
                    to_remove.append(name)
                    continue
            else:
                lig = read_molecule(os.path.join(self.pdbbind_dir, name, f'{name}_ligand.sdf'),
                                    sanitize=True, remove_hs=self.remove_h)
                if lig is None:  # read mol2 file if sdf file cannot be sanitized
                    lig = read_molecule(os.path.join(self.pdbbind_dir, name, f'{name}_ligand.mol2'),
                                        sanitize=True,remove_hs=self.remove_h)
            if self.only_polar_hydrogens:
                for atom in lig.GetAtoms():
                    if atom.GetAtomicNum() == 1 and \
                    [x.GetAtomicNum() for x in atom.GetNeighbors()] == [6]:
                        atom.SetAtomicNum(0)
        ...
        if self.bsp_proteins:
            rec_paths = [os.path.join(self.bsp_dir, name, 'Rec.pdb') for name in complex_names]
        else:
            rec_paths = [os.path.join(self.pdbbind_dir, name,
                        f'{name}_protein_processed.pdb') for name in complex_names]

        if not os.path.exists(self.processed_dir):
            os.mkdir(self.processed_dir)

        if (not os.path.exists(os.path.join(self.processed_dir, 'rec_graphs.pt')) or \
            not os.path.exists(os.path.join(self.processed_dir, 'pocket_and_rec_coords.pt')) or \
            (not os.path.exists(os.path.join(self.processed_dir, self.rec_subgraph_path)) and \
            self.rec_subgraph)):
            log('Get receptors, filter chains, and get its coordinates')
            receptor_representatives = pmap_multi(get_receptor, zip(rec_paths, ligs),
                                        n_jobs=self.n_jobs, cutoff=self.chain_radius,
                                        desc='Get receptors')
            (recs, recs_coords, c_alpha_coords,
            n_coords, c_coords) = map(list, zip(*receptor_representatives))
        ...
        if not os.path.exists(os.path.join(self.processed_dir, 'pocket_and_rec_coords.pt')):
            ...
            pockets_coords = pmap_multi(get_pocket_coords, zip(ligs, recs_coords),
                                        n_jobs=self.n_jobs, cutoff=self.pocket_cutoff,
                                        pocket_mode=self.pocket_mode, desc='Get pocket coords')
            recs_coords_concat = [torch.tensor(np.concatenate(rec_coords, axis=0)) for rec_coords \
                                    in recs_coords]
            ...
            log('pocket_and_rec_coords.pt already exists. \
                Using those instead of creating new ones.')
        ...
        if not os.path.exists(os.path.join(self.processed_dir, 'rec_graphs.pt')):
            ...
            rec_graphs = pmap_multi(get_rec_graph,
                                    zip(recs, recs_coords, c_alpha_coords, n_coords, c_coords),
                                    n_jobs=self.n_jobs,
        ...
        if self.lig_predictions_name is not None:
            ligs_coords = torch.load(os.path.join('data/processed',
                            self.lig_predictions_name))['predictions'][:len(ligs)]
        else:
            ligs_coords = [None] * len(ligs)
        if self.rec_subgraph and not os.path.exists(
                                        os.path.join(self.processed_dir, self.rec_subgraph_path)):
        ...
        if not os.path.exists(os.path.join(self.processed_dir, self.lig_graph_path)):
            ...
            if self.multiple_rdkit_conformers:
                lig_graphs = pmap_multi(get_lig_graph_multiple_conformer, zip(ligs,complex_names),
                                    n_jobs=self.n_jobs,max_neighbors=self.lig_max_neighbors,
                ...
                lig_graphs = pmap_multi(get_lig_graph_revised, zip(ligs,complex_names),
                                    n_jobs=self.n_jobs,
        ...
        if not os.path.exists(os.path.join(self.processed_dir, 'lig_structure_graphs.pt')) and \
            self.lig_structure_graph:
            log('Convert ligands to graphs')
            graphs_masks_angles = pmap_multi(get_lig_structure_graph, zip(ligs),
            n_jobs=self.n_jobs, desc='Get ligand structure graphs with angle information')
        ...
        if not os.path.exists(os.path.join(self.processed_dir, 'geometry_regularization.pt')):
            ...
            save_graphs(os.path.join(self.processed_dir,
                        'geometry_regularization.pt'), geometry_graphs)
        else:
        ...
        if not os.path.exists(os.path.join(self.processed_dir, 'geometry_regularization_ring.pt')):
            ...
            save_graphs(os.path.join(self.processed_dir,
                        'geometry_regularization_ring.pt'), geometry_graphs)
        else:
            log('geometry_regularization.pt already exists or is not needed.')
