...
class HardSampler(Sampler[List[int]]):
    def __init__(self, data_source: Dataset, batch_size: int,
                valid_indices=None, replacement: bool = False,
                num_hard_samples=2,
        ...
        self.standard_sampler = RandomSampler(data_source=Subset(self.data_source, valid_indices),
    ...
    def __iter__(self):
        ...
        for idx in self.standard_sampler:
            if len(batch)<= self.num_hard_samples and len(self.current_hard_indices) >= 0:
                batch.append(self.current_hard_indices[torch.randint(low=0,
                                    high=len(self.current_hard_indices),size=(1,))])
            else:
                batch.append(idx)
