...
class PearsonR(nn.Module):
    """
    Takes a single target property of the QM9 dataset,
    denormalizes it and turns in into meV from eV if it  is an energy
    """
    ...
    def forward(self, preds, targets):
        # concatenate tensors if list of tensors
        preds, targets = concat_if_list(preds), concat_if_list(targets)
...
class RMSD(nn.Module):
    ...
    def forward(self, ligs_coords_pred: List[Tensor], ligs_coords: List[Tensor]) -> Tensor:
        ...
        for lig_coords_pred, lig_coords in zip(ligs_coords_pred, ligs_coords):
            rmsds.append(torch.sqrt(torch.mean(torch.sum(
                ((lig_coords_pred - lig_coords) ** 2), dim=1))))
...
class CentroidDist(nn.Module):
    ...
    def forward(self, ligs_coords_pred: List[Tensor], ligs_coords: List[Tensor]) -> Tensor:
        distances = []
        for lig_coords_pred, lig_coords in zip(ligs_coords_pred, ligs_coords):
            distances.append(torch.linalg.norm(lig_coords_pred.mean(dim=0)-
