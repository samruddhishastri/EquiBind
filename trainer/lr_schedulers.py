...
class WarmUpWrapper:
    ...
    def __init__(self, optimizer, wrapped_scheduler, warmup_steps, interpolation='linear',
        ...
        :param warmup_steps: is a list containing how many warmup steps should be done
                            for each param group before updating all parameters
        :param interpolation:
        :param kwargs:
        '''
        ...
        for param in self.optim.param_groups:
            self.start_lrs.append(param['lr'])
            param['lr'] = 0
    ...
    def step(self, metrics=None):
        ...
        if self._step < self.total_warmup_steps:
            ...
            for steps in self.warmup_steps.cumsum():
                if self._step >= steps:
                    warmup_phase += 1
            for i, param in enumerate(self.optim.param_groups):
                # update all parameters if there is only one entry specified for the warmup steps
                # otherwise only update the ones corresponding to the current warmup phase
                if i <= warmup_phase or len(self.warmup_steps) == 1:
                    # interpolate between 0 and the final starting learning rate
                    interpolation_value = self._step - ([0] +
                                            list(self.warmup_steps.cumsum()))[warmup_phase] +1
                    if self.warmup_steps[warmup_phase] == 0:
                        param['lr'] = self.start_lrs[i]
                    else:
                        if self.interpolation == 'linear':
                            param['lr'] = (self.start_lrs[i] *
                                            (interpolation_value / self.warmup_steps[warmup_phase]))
                        elif self.interpolation == 'cosine':
                            param['lr'] = (self.start_lrs[i] * ((-np.cos((np.pi) *
                            (interpolation_value / self.warmup_steps[warmup_phase])) + 1) * 0.5))
                        else:
    ...
    def state_dict(self):
        ...
        state_dict['wrapped_scheduler'] = self.wrapped_scheduler.state_dict()
        return state_dict
